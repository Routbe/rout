...
export async function removeShortLink(id: string) {
  await supabaseAdmin.from("qr_scans").delete().eq("tracked_qr_id", id);
  const { error } = await supabaseAdmin.from("tracked_qrs").delete().eq("id", id);
  if (error) throw new Error(error.message);
  return { ok: true as const };
}
...
export async function purgeScans(trackedQrId: string, olderThanDays: number | null) {
  let query = supabaseAdmin.from("qr_scans").delete().eq("tracked_qr_id", trackedQrId);
...
export async function fetchBadges(): Promise<AdminBadge[]> {
  ...
  const { data: held } = await supabaseAdmin.from("user_badges").select("badge_id");
  const counts = new Map<string, number>();
